...
import purdue_add_headers

if __name__ == '__main__':
    if os.path.isdir('files_with_headers'):
        shutil.rmtree('files_with_headers')
    metadata = pandas.read_csv('pslw_test_metadata.csv')
    ## This line generates the output.
    purdue_add_headers.add_headers_recursive('pslw_files_without_headers', metadata, 'blackboard')
    ...
    for dirpath, dirnames, files in os.walk('files_with_headers'):
        for name in files:
            ...
            if '.txt' not in name:
                continue
            expected = dirpath.replace('files_with_headers', 'pslw_expected_test_output')
