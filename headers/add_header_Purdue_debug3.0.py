...
def add_header_to_file(filename, master, overwrite=False): # filename = folder1.../Lan/Lan_p1d3/WA_Second Draft_lan12_attempt_2017-06-29_Lan Ge_WA Second.txt
    ...
    if '.txt' in filename:
        ...
        master_data_frame = pandas.DataFrame(master)
        #career_account_column = master_data_frame['User_ID']
        career_account_list = master_data_frame['User_ID'].tolist()

        for career_account in career_account_list:
            if not re.search(career_account, filename):
                print(">>>>> add header manual to:", filename)
            else:
                ...
                print('adding headers to the file', filename)
                
                #global textfile
                #nonlocal textfile
                textfile = open(filename, 'r') # unbound error: referenced before assignment
                ...
                # check: line-42 at https://github.com/writecrow/text_processing/blob/master/repository/repository_file_names_revised.py
