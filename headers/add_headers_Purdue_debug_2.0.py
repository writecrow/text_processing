...
def add_header_to_file(filename, master, overwrite=False): # filename = folder1.../Lan/Lan_p1d3/WA_Second Draft_lan12_attempt_2017-06-29_Lan Ge_WA Second.txt
    ...
    if '.txt' in filename:
        ...
        # Warning: check if all filenames have career_account as "_lan12_"
        if re.match(r'_([a-z]+[0-9]+)_',filename):
            # only assign "[a-z]+[0-9]+" to career account (lan12)
            # there might be a problem if a string in the filename contains digits and characters *
            career_account = match.group(1)
        else:
            # this condition reports any files who career account can't be found based on the their file names
            # there are exceptions, for example, a small set of career account has only lowercase letters w/o numebers
            # these are the files that we need to manually add headers
            print(filename,">>> Mannual check: unable to identify career account from the filename.")
        ...
        # This condition reports the rows in filtered_master that has no row values filled in
        # This means we need to look into these files to manually figure out the reasons.
        if filtered_master.empty: 
            print(filename,'*** Manual check: uable to find metadata for this file')
        else:
            ...
            course = filtered_master['COURSE_NUMBER']
            ...
            country_code = filtered_master['Country Code'].to_string(index=False)
            ...
            # To-do: change this path format for windows pattern
...
def add_headers_recursive(directory, master, overwrite=False):
    ...
    for dirpath, dirnames, files in os.walk(directory): #dirpath = whole path, dirnames = folder names, file = filename.txt (with txt)
        for name in files: #with each filename.txt in the files list (WA_second Draft_lan12_attempt_2017-06-29_Lan Ge_WA Second.txt)
            is_this_a_text_file = add_header_to_file(os.path.join(dirpath, name), master, overwrite) #dirpath join name together = folder/folder2/file's name.txt
